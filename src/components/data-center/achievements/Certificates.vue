    ...
    <div class="content-section">
      ...
      <div class="section-header">
        <div class="stats-overview">
          ...
          <div 
            class="stat-item clickable-tab" 
            :class="{ 'tab-active': selectedFilter === 'teacher-title' }"
            @click="selectedFilter = 'teacher-title'"
          >
            <span class="stat-label">教师职称：</span>
            <span class="stat-value">{{ getCountByType('teacher-title') }}</span>
          </div>
          <div 
            class="stat-item clickable-tab" 
            :class="{ 'tab-active': selectedFilter === 'professional-cert' }"
            @click="selectedFilter = 'professional-cert'"
          >
            <span class="stat-label">职业资格：</span>
            <span class="stat-value">{{ getCountByType('professional-cert') }}</span>
          </div>
          <div 
            class="stat-item clickable-tab" 
            :class="{ 'tab-active': selectedFilter === 'practice-cert' }"
            @click="selectedFilter = 'practice-cert'"
          >
            <span class="stat-label">执业资格：</span>
            <span class="stat-value">{{ getCountByType('practice-cert') }}</span>
          </div>
        </div>
      ...
      <div class="content-area">
        ...
        <div v-if="viewMode === 'list'" class="list-view">
          <div class="table-header">
            ...
            <div class="col-issue">颁发时间</div>
            <div class="col-valid">有效时间</div>
          ...
          <div class="table-body">
            ...
            >
              ...
              <div class="col-number">{{ cert.number }}</div>
              <div class="col-org">{{ cert.organization }}</div>
              <div class="col-type">
                <span 
                  class="type-badge" 
              ...
              <div class="col-issue">{{ cert.issueDate }}</div>
              <div class="col-valid">{{ cert.validUntil }}</div>
              <div class="col-status">
                <span 
                  class="status-badge" 
                  :class="{ 
                    'status-valid': cert.status === 'valid',
                    'status-expired': cert.status === 'expired'
                  }"
                >
          ...
          >
            ...
            <div class="card-content">
              <div class="info-row">
                ...
                <span class="value">{{ cert.number }}</span>
              </div>
              <div class="info-row">
    ...
    <div v-if="showCertificateModal" class="modal-overlay" @click="closeCertificateModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          ...
          <button class="close-btn" @click="closeCertificateModal">×</button>
        </div>
        <div class="modal-body">
          ...
          <div class="certificate-info">
            <div class="info-item">
              <span class="label">证书编号：</span>
              <span class="value">{{ selectedCertificate?.number }}</span>
            </div>
            <div class="info-item">
              <span class="label">颁发机构：</span>
              <span class="value">{{ selectedCertificate?.organization }}</span>
            </div>
            <div class="info-item">
              <span class="label">证书类型：</span>
              <span class="value">
                <span 
                  class="type-badge" 
                  :class="getTypeBadgeClass(selectedCertificate?.type)"
                >
            ...
            <div class="info-item">
              <span class="label">颁发时间：</span>
              <span class="value">{{ selectedCertificate?.issueDate }}</span>
            </div>
            <div class="info-item">
              <span class="label">有效时间：</span>
              <span class="value">{{ selectedCertificate?.validUntil }}</span>
            </div>
            <div class="info-item">
              <span class="label">附件资料：</span>
              <div class="docs-list">
                <span 
                  v-for="(doc, index) in selectedCertificate?.documents" 
                  :key="index"
                  class="doc-item"
                  @click="viewDocument(doc)"
                >
...
const getCountByType = (type) => {
  return certificates.value.filter(cert => cert.type === type).length
}
...
const certificateTypeMap = {
  'teacher-title': '教师专业职称',
  'professional-cert': '职业资格证书',
  'practice-cert': '执业资格证书'
}
...
const certificates = ref([
  {
    ...
    image: '/pic/data.png',
    documents: [
      { name: '教师资格证.pdf', size: '1.2MB', uploadTime: '2024-01-10' },
      { name: '体检报告.pdf', size: '856KB', uploadTime: '2024-01-12' }
    ]
  },
  {
    id: 2,
    name: '计算机技术与软件专业技术资格证',
    number: 'IT2024002',
    ...
    image: '/pic/data.png',
    documents: [
      { name: '软考证书.pdf', size: '1.5MB', uploadTime: '2023-06-18' },
      { name: '成绩单.pdf', size: '2.1MB', uploadTime: '2023-06-22' }
    ]
  },
  {
    id: 3,
    ...
    image: '/pic/data.png',
    documents: [
      { name: 'CPA证书.pdf', size: '1.8MB', uploadTime: '2022-12-08' }
    ]
  },
  {
    id: 4,
    ...
    image: '/pic/data.png',
    documents: [
      { name: '建造师证书.pdf', size: '2.3MB', uploadTime: '2024-03-01' },
      { name: '继续教育证明.pdf', size: '1.1MB', uploadTime: '2024-03-15' }
    ]
  }
])
...
const filteredCertificates = computed(() => {
  ...
  if (searchKeyword.value) {
    ...
    filtered = filtered.filter(cert => 
      ...
      cert.number.toLowerCase().includes(keyword) ||
...
const getCertificateTypeName = (type) => {
  return certificateTypeMap[type] || '未知类型'
}

const getTypeBadgeClass = (type) => {
  const typeClassMap = {
    'teacher-title': 'type-teacher',
    'professional-cert': 'type-professional',
    'practice-cert': 'type-practice'
  }
...
.table-header {
  display: grid;
  grid-template-columns: 2fr 1.5fr 1.5fr 1.2fr 1fr 1fr 0.8fr 2fr 0.8fr 0.8fr;
  gap: 16px;
  padding: 16px 24px;
  background: #f8fafc;
  border-bottom: 1px solid #e2e8f0;
